...
} else if ($subdomain == 'fullstack') {
    $consumerKey = env('FULLSTACK_CONSUMER_KEY');
    $consumerSecret = env('FULLSTACK_CONSUMER_SECRET');
    $accessToken = env('FULLSTACK_ACCESS_TOKEN');
    $accessTokenSecret = env('FULLSTACK_ACCESS_TOKEN_SECRET');   
} else if ($subdomain == 'nhl') {
    $consumerKey = env('NHL_CONSUMER_KEY');
    $consumerSecret = env('NHL_CONSUMER_SECRET');
    $accessToken = env('NHL_ACCESS_TOKEN');
    $accessTokenSecret = env('NHL_ACCESS_TOKEN_SECRET');  
} else {
    /*
