...
class Twitter extends tmhOAuth {
    ...
    //public static $baseUrlSearch = 'https://twitter.com/search?q=%23';
    
    public $baseUrlHashtag = 'https://twitter.com/hashtag/';
    ...
    public function setBaseUrlHashtag($url)
    {
        $this->baseUrlHashtag = $url;
    }
    
    public function setInstagram()
    {
        $this->setBaseUrl('https://instagram.com/');
        $this->setBaseUrlHashtag('https://instagram.com/explore/tags/');
        return $this;
    }
    ...
    public function setTwitter()
    {
        ...
        $this->setBaseUrlHashtag('https://twitter.com/hashtag/');
        return $this;
    }
	...
	public function linkify($tweet)
	{
		...
		if ($type == 'text')
		{
			...
			$text   = preg_replace_callback('#'.$patterns['url'].'#i', function($matches)
			{
                ...
                ///////////////////////////////////////////
                // just the domain name in the linked text
                //
                preg_match("~(https?://www\.|https?://|www\.)?([^/]+)~i", $input, $arr);
                if (!empty($arr[2])) {
                    $input = $arr[2];
                    $input.="&raquo;";
                }
		...
		// User
		$text = preg_replace('/'.$patterns['user'].'/i', " <a href=\"" . $this->baseUrl . "\\1\" target=\"_blank\">@\\1</a>", $text);
   		//$text = preg_replace('/'.$patterns['user'].'/i', " <a href=\"https://twitter.com/\\1\" target=\"_blank\">@\\1</a>", $text);

		// Hashtag
		$text = preg_replace('/'.$patterns['hashtag'].'/ui', "<a href=\"" . $this->baseUrlHashtag . "\\1\" target=\"_blank\">#\\1</a>", $text);
