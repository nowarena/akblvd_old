$(document).ready(function() {
    ...
    function displayEndMsg(memberId, index) {
        ...
        $("#footerCont_" + childId + "_" + index + " > .ageLink").html('&nbsp;');
        $("#expandLink_" + childId + "_" + index).css('display','');
        if (index == 1) {
            displayBlank(childId, 2);
            displayBlank(childId, 3);
        } else if (index == 2) {
            displayBlank(childId, 3);
        }
    ...
    $(".expandLink").click(function() {
        ...
        //console.log('childid:'+childId+'boxNum:'+boxNum);
    ...
    function getJson(memId, lastSocialMediaId, childId) {
    ...
    $(".navRight").click(function() {
        ...
        if (contentArr[memId].length > numMediaDisplayed) {
            ...
            for(j in contentArr[memId]) {
                ...
                j = parseInt(j);
                if (obj['id'] == lastSocialMediaId && contentArr[memId].length > i) {
                    hasMediaNotDisplayed = true;
                    subMemberArr = [];
                    subMemberArr[memId] = contentArr[memId].slice(i, i + numMediaDisplayed);
                    break;
                }
            }
        ...
        if (hasMediaNotDisplayed) {
            ...
            getJson(memId, memberIdSocialIdArr[memId].lastSocialMediaId, childId);
        }
        ...
        if (memberIdSocialIdArr[memId].firstSocialMediaId > 0) {
            for(var j in contentArr[memId]) {
                if (contentArr[memId][j]['id'] == memberIdSocialIdArr[memId].firstSocialMediaId) {    
                    ...
                    console.log(subMemberArr[memId]);
                    displayChildRow(memId, childId, subMemberArr);
                }
            }
        }
    ...
    $(".childNext, .childPrev").click(function() {
        ...
        $('.next_child_id_' + childId).text(memberName);

        // set the next member id to be visible
        childIdMemIdVisibleArr[childId] = newMemberId;

        contentArr[newMemberId] = [];
        getJson(newMemberId, 0, childId); 
        // name, avatar
    ...
    mngResize();
    mngChildNextLinks();
    // display members in parent layout
    ...
    function displayChildRow(memId, childId, contentArr) {
        ...
        for(var i in contentArr[memId]) {
            ...
            //console.log(contentArr[memId][i]['text']);
